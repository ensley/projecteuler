...
```{r}
(m_pal <- microbenchmark::microbenchmark(is_palindrome(468841),
...
As we would expect, `is_palindrome3` performs the best because it quits as soon as it encounters digits that do not match. This will probably come in handy, since there are more non-palindromes than palindromes, so we'll 


```{r}
microbenchmark::microbenchmark(is_palindrome(12345),
                               is_palindrome2(12345))
```
...
```{r}
microbenchmark::microbenchmark(p4.1(), p4.2())
```
